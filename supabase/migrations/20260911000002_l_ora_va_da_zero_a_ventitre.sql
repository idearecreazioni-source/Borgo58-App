-- =====================================================================
-- L'ORA DELL'IMPEGNO VA DA 00:00 A 23:59
-- =====================================================================
-- 11/09/2026. Rilievo della revisione del diff su `20260911000001`: il
-- controllo della forma `^\d{1,2}:\d{2}(:\d{2})?$` lasciava passare «24:00»,
-- che PostgreSQL accetta come `time` — e l'impegno sarebbe nato con un'ora
-- che non esiste in un giorno.
--
-- ⚠️ SI CORREGGE CON UNA MIGRAZIONE NUOVA, e non riscrivendo quella di
--    stamattina, anche se è andata solo sul progetto di prova: una
--    migrazione applicata non si riscrive mai (regola del 23/08).
--
-- ⚠️ Solo l'ora dell'IMPEGNO: il controllo dell'avviso (`avviso_ora`, dal
--    10/09) ha la stessa forma e non è toccato — non era nel perimetro, ed
--    è dichiarato nel riepilogo.
-- =====================================================================

-- I due corpi, ripresi dal database di PROVA (che ha già la 20260911000001).
-- `create or replace` con la stessa firma conserva proprietario e permessi.
...
begin
  ...
  if p_tipo = 'promemoria' then
    ...
    if v_ora is not null then
      begin
        -- ⚠️ ORE 00-23 E MINUTI 00-59, SCRITTI (revisione del diff, 11/09):
        --    «24:00» ha la forma di un'ora e PostgreSQL la accetta come
        --    `time`, ma non è l'ora di un impegno.
        if v_ora !~ '^([01]?\d|2[0-3]):[0-5]\d(:[0-5]\d)?$' then
...
begin
  case p_tipo
    ...
    when 'promemoria' then
      ...
      -- ⚠️ Ore 00-23, minuti 00-59: «24:00» si rifiuta (revisione, 11/09).
      if v_ora_imp is not null and v_ora_imp !~ '^([01]?\d|2[0-3]):[0-5]\d(:[0-5]\d)?$' then
...
declare
  ...
  v_r     jsonb;
  v_preso boolean;
  v_dom   date := (now() at time zone 'Europe/Rome')::date + 30;
  v_id    uuid;
  v_ora   time;
begin
  -- «24:00» non è un'ora dell'impegno: resta visibile, non si scrive
  v_r := voce_risolvi_dati('promemoria', jsonb_build_object('titolo', 'VERIFICA-11SET-2 a', 'ora', '24:00'));
  if v_r->'dati' ? 'ora' then
    raise exception 'VERIFICA: «24:00» è rimasta come ora.';
  end if;
  if v_r->'dati'->>'ora_non_capita' is distinct from '24:00' then
    raise exception 'VERIFICA: «24:00» non si vede come ora non capita.';
  end if;
  -- …e i minuti oltre 59 nemmeno
  v_r := voce_risolvi_dati('promemoria', jsonb_build_object('titolo', 'VERIFICA-11SET-2 b', 'ora', '10:60'));
  if v_r->'dati' ? 'ora' then
    raise exception 'VERIFICA: «10:60» è rimasta come ora.';
  end if;
  -- Le ore giuste passano ancora, anche ai bordi
  v_r := voce_risolvi_dati('promemoria', jsonb_build_object('titolo', 'VERIFICA-11SET-2 c', 'ora', '23:59'));
  if v_r->'dati'->>'ora' is distinct from '23:59' then
    raise exception 'VERIFICA: «23:59» non passa più.';
  end if;
  v_r := voce_risolvi_dati('promemoria', jsonb_build_object('titolo', 'VERIFICA-11SET-2 d', 'ora', '9:30'));
  if v_r->'dati'->>'ora' is distinct from '09:30' then
    raise exception 'VERIFICA: «9:30» non diventa più «09:30».';
  end if;
  -- L'esecutore rifiuta «24:00» da sé
  v_preso := false;
  begin
    perform fai_azione_dettata('promemoria', jsonb_build_object(
              'titolo', 'VERIFICA-11SET-2 esecutore', 'data', v_dom::text, 'ora', '24:00'));
  exception when others then
    v_preso := true;
  end;
  if not v_preso then
    raise exception 'VERIFICA: l''esecutore ha scritto un impegno alle 24:00.';
  end if;
  -- …e scrive ancora le ore giuste
  v_r := fai_azione_dettata('promemoria', jsonb_build_object(
           'titolo', 'VERIFICA-11SET-2 buona', 'data', v_dom::text, 'ora', '00:00'));
  v_id := (v_r->>'task_id')::uuid;
  select due_time into v_ora from tasks where id = v_id;
  delete from tasks where id = v_id;
  if v_ora is distinct from '00:00'::time then
    raise exception 'VERIFICA: «00:00» non è stata scritta (è %).', v_ora;
  end if;
  if exists (select 1 from tasks where title like 'VERIFICA-11SET-2%') then
    raise exception 'VERIFICA: sono rimasti impegni di prova.';
  end if;
  perform pretendi_nessun_residuo(v_foto, 'la verifica delle ore da 00 a 23');
  raise notice 'L''ora dell''impegno va da 00:00 a 23:59: «24:00» e «10:60» non si scrivono.';
end $verifica$;

insert into applied_migrations (version, name)
values ('20260911000002', 'l_ora_va_da_zero_a_ventitre') on conflict (version) do nothing;

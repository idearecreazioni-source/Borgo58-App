...
- 🔄 **Il carico da fattura** — costruito e installato il 12/08 (vedi «Chiuso di recente»). **Chiuso quando** una fattura vera arriva a `info@`, l'assistente propone il carico e Alessio lo conferma. ⚠️ La prova lascia dietro di sé lotti di magazzino e righe del registro HACCP **veri**: vanno cancellati subito dopo (§5 punto 8), e un registro HACCP con dentro una consegna mai avvenuta è peggio di un registro vuoto.
...
- **71 migrazioni** registrate in `applied_migrations` (le ultime del 12/08: `carico_con_memoria`, `ogni_rincaro_si_vede`, `varianti_e_convenienza`, `una_mail_non_si_ripaga_per_sempre`). Verificato in produzione dopo ognuna, dal connettore in sola lettura.

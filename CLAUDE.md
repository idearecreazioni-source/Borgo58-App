...
- 🔄 **Email automatica al cliente quando Alessio conferma** (scelta del 10/08) — **scritta l'11/08**, migrazione `20260811000001` e funzione online `email-cliente` (Resend, piano gratuito, regione Irlanda). Tre decisioni dentro: nasce **spenta** (`service_settings.email_conferma_attiva`) perché un mittente non ancora verificato finisce nello spam e rovina la reputazione di tutti i messaggi dopo; il **registro degli invii non contiene l'indirizzo** e muore con la prenotazione (`on delete cascade`), altrimenti sopravviverebbe alla pulizia dei dati clienti rendendola finta; la **decisione è separata dall'invio** (`email_conferma_dovuta()` / `invia_email_conferma()`) così la migrazione prova la regola senza spedire niente a nessuno. Se l'invio fallisce arriva un **avviso su Telegram**: un'email di conferma che non parte è invisibile per definizione. **Chiuso quando** Alessio installa la funzione, applica la migrazione, accende l'interruttore e una prova vera arriva davvero in casella.

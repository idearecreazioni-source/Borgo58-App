...
**Chiuso di recente:**
- **Posti liberi in tempo reale sul form pubblico (10/08)** — quattro decisioni di Alessio: orario libero ogni 15 minuti con ultimo ingresso (non turni fissi), **conferma sempre sua** (niente prenotazioni automatiche), richiesta in attesa che tiene il posto, email automatica al cliente quando conferma (da fare, vedi §10). Migrazione `20260810000001`, schermata `Sala e orari`, 3 prove automatiche nuove.
...
**In capo ad Alessio:**
- **Compilare "Sala e orari"** (coperti veri di ogni tavolo — ora sono tutti a 2 per finta, giorni e orari di apertura) e poi accendere l'interruttore delle prenotazioni online
...
- **48 migrazioni** registrate in `applied_migrations` (le ultime: le sei del 09/08 — `chiusura_sconto_omaggio_atomica`, `blocco1_cessioni_e_mance`, `blocco2_record_e_promemoria`, `blocco3_cancellazioni_con_promemoria`, `blocco4_rifiniture_atomiche`, `blindatura_notifiche` — e `capienza_e_orari` del 10/08)
- **14 righe in `service_hours`** e i coperti dei tavoli: al 10/08 sono ancora **valori segnaposto (2 per tavolo, tutti i servizi spenti)**. Non sono i numeri veri del locale finché Alessio non compila "Sala e orari".

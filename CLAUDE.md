...
- 🔴 **Il freno anti-tempesta sta zittendo rincari veri** — trovato il 13/08 verificando gli avvisi dopo la terza fattura di collaudo: `segnala_allarme()` lascia passare **un avviso per tipo all'ora**, e il tipo è `rincaro_<ingrediente>`. Alessio ha confermato due fatture a venti minuti di distanza e il **secondo rincaro sullo stesso prodotto non è mai arrivato su Telegram** (in schermata sì, perché lì il calcolo è dal vivo e non passa dal freno). Il freno è giusto per **un guasto che si ripete**, dove il secondo messaggio non aggiunge niente; **un rincaro è un fatto nuovo ogni volta**. Correzione: mettere il prezzo nel tipo dell'avviso (`rincaro_<ingrediente>_<prezzo>`), così lo stesso rincaro non si ripete e uno diverso passa. Da fare **prima** che arrivino fatture vere — nella vita reale due fatture dello stesso prodotto a venti minuti non capitano, ma un rincaro perso in silenzio è esattamente ciò che questo modulo esiste per evitare.
...
- **72 migrazioni** registrate in `applied_migrations` (le ultime del 12-13/08: `ogni_rincaro_si_vede`, `varianti_e_convenienza`, `una_mail_non_si_ripaga_per_sempre`, `niente_ingredienti_doppi`). Verificato in produzione dopo ognuna, dal connettore in sola lettura.

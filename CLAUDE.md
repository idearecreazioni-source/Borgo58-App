...
- **La memoria delle diciture e la sorveglianza dei prezzi (12/08)** — nate da due obiezioni di Alessio al carico da fattura, e sono quelle che hanno **cambiato il senso del modulo**: *«la giacenza mi interessa poco, il locale è piccolo e vedo a occhio; ma se un fornitore aumenta un prezzo senza dirmelo voglio saperlo»*. **Il valore non è il magazzino, è il prezzo.** Migrazioni `20260812000013` e `…14`.
  ...
  - **Soglia 10% in `service_settings`** (dato, non codice) e **`ingredients.prezzo_stagionale`** che zittisce l'avviso su ortofrutta e pesce: senza, a novembre suonerebbe metà della spesa e si smetterebbe di leggerlo — la lezione della sentinella applicata *prima* di sbagliare.
...
- **68 migrazioni** registrate in `applied_migrations` (le ultime del 12/08: `carico_da_fattura`, `haccp_alla_porta`, `memoria_articoli`, `carico_con_memoria`). Verificato in produzione dopo ognuna, dal connettore in sola lettura.

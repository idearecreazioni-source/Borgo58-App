...
| C4 | 🔴 **Il pulsante «il cliente vuole fattura» in Comande.** Lo stato `fattura_da_emettere` **esiste già** nel database ed è **già isolato nei conteggi** — ma **dalla sala non lo si raggiunge**: manca il gesto alla chiusura del conto, e manca la strada per chiuderlo quando la fattura viene emessa. *Un cliente che chiede la fattura al tavolo è un caso normale, e oggi in sala non c'è niente da premere.* ⚠️ **Il testo qui sopra è dell'origine e dal 31/08 non è più vero**: il gesto in sala c'è, e da *Cassa → Incassato e scontrinato* il conto si chiude col numero vero. Restava scoperto il resto — vedi la colonna di stato. | 30/08/2026 | Sua richiesta | si può fare adesso | fatta · gesto in sala dal 31/08; il 22/09 la spunta è stata spostata SOPRA «Paga contante» (stava sotto, contro quel che dichiarava) e messa sotto prove |
| C5 | **La chiusura dell'anno fiscale, con avviso se restano conti senza documento.** Riusa il meccanismo delle chiusure mensili, che esiste già e tiene lo storico delle chiusure precedenti senza riscrivere il passato. | 30/08/2026 | Sua richiesta | si può fare adesso | in corso · proposta aperta e non unita, migrazioni `20260923000003` e `20260923000004` **applicate al solo progetto di prova**, riepilogo [la chiusura dell'anno fiscale](consegne/20260923_la_chiusura_dell_anno_fiscale.md). 🔴 **Il quesito L21 resta APERTO e il gestionale non lo decide**: i conti senza documento vengono contati, mostrati e lasciati dove sono, e la chiusura si rifiuta di avvenire finché il titolare non conferma di averli visti. ⚠️ **C6 e C7 non sono in questo lavoro** — la loro forma la deve dire la commercialista (L22) |
...
| C11 | 🔴 **L'ETICHETTA «INVESTIMENTO» E IL COSTO DEL PROGETTO.** Oggi nessun numero dice **quanto è costato mettere in piedi il locale**: `cash_movements` ha 26 colonne e nessuna etichetta, quindi un forno da 6.000 € e una bolletta della luce sono indistinguibili, e il totale più vicino (`rettifiche_fiscali → costi_totali`, in *Fiscale → Deducibilità*) è **per anno civile e per una società sola**, e **esclude la Tasca**. Le decisioni sono già prese e sono quattro: **(a)** è un'**etichetta manuale su ogni uscita di cassa**, non una sezione separata — una sezione creerebbe due verità sulla stessa spesa e andrebbe smontata dopo l'apertura, un'etichetta si smette solo di usare; **(b)** la vista è **divisa per soggetto — Borgo 58, la Tasca, e il totale** — perché *un numero solo direbbe quanto è costato aprire nascondendo chi l'ha pagato*; **(c)** «in cosa» e «con quali soldi» **non vogliono campi nuovi**: la causale e la nota dicono già in cosa, il mezzo e il soggetto dicono già con quali soldi; **(d)** ⚠️ **serve fino a marzo 2027 e poi decade** — non costruirci intorno niente di permanente. ⚠️ **`tag_anticipazioni` NON è il posto**: sono le etichette delle spese che la società *rimborsa*, e un investimento non è un rimborso. 🔴 **Il lavoro era stato cominciato e riportato indietro il 31/08 su sua richiesta** («fermati al primo punto pulito»): migrazione `20260831000012` tolta dal repository, colonna e funzioni a **zero** su prova e produzione, niente lasciato a metà. ⚠️ **E la voce era sparita da ogni coda**: viveva solo dentro il riepilogo del 31/08, che è esattamente il difetto per cui questo file esiste — rimessa qui il 21/09/2026 **senza avviare nessuna implementazione**. Dipendenza: il quesito **L19** alla commercialista (gli acquisti con fattura fatti prima della partita IVA sono recuperabili, e come vanno intestati) cambia cosa può entrare nel totale. | 31/08/2026 | Sua decisione, riportata nei riepiloghi del 31/08 ([la regola della stima](consegne/20260831_la_regola_della_stima.md), Blocco 6 dei [sette mondi](consegne/20260831_i_sette_mondi_e_le_schermate_cieche.md)), in [SPEC-0004](specifiche/SPEC-0004-costo-effettivo-del-progetto.md), e costruita col riepilogo [l'etichetta «investimento»](consegne/20260921_l_etichetta_investimento.md) | si può fare adesso | fatta · proposta #121 **unita in `slave`** e migrazione `20260921000003` **applicata al solo progetto di prova** il 22/09/2026. L'etichetta vive in due posti — `cash_movements` e `anticipazioni_socio`, perché un investimento anticipato per conto della società non passa dalla prima nota — e la vista è *Cassa → Quanto è costato il progetto*. ⚠️ **La produzione resta fuori**, e quando ci arriverà lo dirà `npm run migra`, non questa riga. ⚠️ **Nessun collaudo con le mani**: quello che è provato è provato da migrazione e prove automatiche, e nessun dato è stato creato per guardare |

...
| Comande | **Parziale**: Sala rifatta e verificata dal vivo (08/08); Bar e Cucina non ancora rifatte (schermo transitorio su `/comande/reparti`); carta dei vini non costruita (manca la fonte dati) | A per lettura/presa ordini; **B1 obbligatorio quando arriva il mini-PC**; **B4 non rispettato oggi in un punto preciso — vedi difetto sotto** |
...
- **Chi fa cosa sul repository — vincolo di processo, non di codice**: solo Alessio esegue `git push` e applica le migrazioni SQL (copiate a mano nell'SQL Editor Supabase, MCP/CLI non funzionanti sulla sua macchina). Nessuna sessione IA esegue mai un push o applica mai una migrazione al posto suo, indipendentemente da quanto la sessione sia sicura che il codice sia corretto.

...
- **Viste `_display` SECURITY DEFINER** (senza `security_invoker`) per nascondere colonne economiche allo staff pur mantenendo l'accesso: la vista gira coi permessi del proprietario e scavalca la RLS delle tabelle sotto, ma espone **solo colonne operative** — il dato sensibile non è nascosto, è strutturalmente assente.
  ...
  - 🔴 **REGOLA, da qui in avanti.** Una vista che legge una tabella titolare-only sta in uno di due stati, e la differenza si dichiara per iscritto:
    ...
    2. **apertura voluta** — ammessa solo se la vista espone **zero colonne economiche** *e* la ragione è scritta nel `comment on view` o nella migrazione che la crea.
  ...
  - **La rete**: `viste_che_scavalcano_rls()` legge il catalogo e le elenca, con l'indicazione di quali espongono colonne di denaro; `tests/app/permessi.test.js` congela l'elenco delle aperture volute e pretende che nessuna esponga denaro. È il terzo elenco congelato del progetto, accanto a quello delle funzioni aperte ad `anon` e di quelle senza portiere — e nasce per la stessa ragione: un elenco che cresce in silenzio non è più un controllo.
  - ⚠️ **STATO AL 04/09/2026, e va letto per quello che è**: la correzione delle due viste è **scritta in una migrazione e non ancora applicata a nessun database** — né alla produzione né al progetto di prova. Quello che è stato provato dal vivo è il **difetto**, non la cura. Finché questa riga non viene aggiornata da chi ha applicato la migrazione, il Contratto descrive la regola che il progetto si dà, non lo stato del gestionale vero.

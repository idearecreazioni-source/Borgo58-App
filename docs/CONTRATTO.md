...
- **Viste `_display` SECURITY DEFINER** (senza `security_invoker`) per nascondere colonne economiche allo staff pur mantenendo l'accesso: la vista gira coi permessi del proprietario e scavalca la RLS delle tabelle sotto, ma espone **solo colonne operative** — il dato sensibile non è nascosto, è strutturalmente assente.
  - 🔴 **LA RIGA DICEVA «verificato oggi: 9 viste di questo tipo nel progetto, espongono solo colonne operative», ED È DIVENTATA FALSA.** Rimisurato dal catalogo il 04/09/2026: le viste senza `security_invoker` erano **dieci**, e **due esponevano colonne economiche** — `v_cash_balance` (saldo di cassa e di banca) e `v_discounts_gifts_monthly` (totali di sconti e omaggi). La frase non era sbagliata quando è stata scritta: è invecchiata, e proprio sul punto in cui il difetto viveva. Chi la leggeva per decidere se una vista era sicura riceveva una rassicurazione non più fondata.
  - **Come è successo, ed è la parte da tenere**: quelle due erano nate **protette** il 02/08 e hanno perso l'opzione il 13/08, a un `create or replace view` che non la ripeteva. **Postgres non conserva le `reloptions` in un `create or replace`, e non solleva alcun errore.** I commenti di quelle due migrazioni discutono il vincolo `42P16` sull'ordine delle colonne: chi scriveva pensava a quello.
  - 🔴 **REGOLA, da qui in avanti.** Una vista che legge una tabella titolare-only sta in uno di due stati, e la differenza si dichiara per iscritto:
    1. **`security_invoker = true`** — è la regola. Obbligatoria per qualunque vista che esponga colonne economiche.
    2. **apertura voluta** — ammessa solo se la vista espone **zero colonne economiche riservate** *e* la ragione è scritta nel `comment on view` o nella migrazione che la crea.
  - 🔴 **«RISERVATE» È UNA PAROLA AGGIUNTA IL 05/09/2026, e non è un ammorbidimento: è la distinzione che mancava.** La riga diceva «zero colonne economiche», e a quella lettera `menu_items_display` violava la regola — espone `selling_price`. Ma **`selling_price` è il prezzo di listino del piatto**: lo legge il cliente sul menu, e senza di esso nessuno in sala può prendere una comanda. Quella vista esiste dal 04/08 apposta per farlo vedere allo staff, e il suo commento lo dichiara da allora: «nome, categoria, prezzo di vendita. Niente food cost/margine — sicura per lo staff».
    - **Cosa resta riservato, e non cambia**: costi d'acquisto, food cost, margini, saldi di cassa e di banca, incassi, imposte, totali di sconti e omaggi, prestiti, caparre. Una vista che scavalca la RLS e mostra allo staff uno di questi resta un difetto.
    - ⚠️ **L'esenzione è UNA COPPIA — `menu_items_display` × `selling_price` — e non un nome di colonna.** Esentare «price» o «prezzo» ovunque toglierebbe dal setaccio proprio il caso per cui il setaccio esiste: una vista che mostra allo staff il **prezzo d'acquisto** di un ingrediente. Una seconda colonna di denaro su `menu_items_display` continua a essere segnalata, e `selling_price` su qualunque altra vista pure — c'è una controprova nella verifica della migrazione `20260905000001` che lo dimostra costruendo il caso invece di affermarlo.
  - ⚠️ **E chi riscrive una di queste viste con `create or replace view` DEVE ripetere `with (security_invoker = true)`**: ometterlo non è un errore che qualcuno vede, è una protezione che sparisce in silenzio. Dove basta cambiare l'opzione senza toccare la definizione, `alter view … set` è preferibile: non c'è nessun corpo da ricopiare.
  - **La rete**: `viste_che_scavalcano_rls()` legge il catalogo e le elenca, dicendo **quali colonne economiche riservate** ciascuna espone — non un sì o un no, i nomi: con un'esenzione dichiarata in mezzo, un «no» non basterebbe a distinguere «non ha colonne di denaro» da «ne ha una, ed è quella dichiarata». `tests/app/permessi.test.js` congela l'elenco delle aperture volute, pretende che nessuna esponga denaro riservato, e pretende anche che **l'esenzione abbia ancora il suo caso**: il giorno che `menu_items_display` diventasse `security_invoker` o perdesse quella colonna, la riga nella rete resterebbe a esentare qualcosa che non esiste più. È il terzo elenco congelato del progetto, accanto a quello delle funzioni aperte ad `anon` e di quelle senza portiere — e nasce per la stessa ragione: un elenco che cresce in silenzio non è più un controllo.
  - ⚠️ **STATO AL 05/09/2026.** La riga diceva, il 04/09: ~~«la correzione delle due viste è scritta in una migrazione e non ancora applicata a nessun database — né alla produzione né al progetto di prova»~~. **Non è più vero, e resta scritto perché il confronto sia possibile**: `20260904000001` è stata applicata al **progetto di prova**, due volte, e lì la cura è stata vista funzionare — lo staff non riceve righe dalle due viste, il titolare sì.
    - 🔴 **In PRODUZIONE non è applicata**, e quindi nel gestionale vero le due viste dei soldi scavalcano ancora la RLS. Chi legge questa riga per sapere com'è il locale, non il progetto, deve leggere questo.
    - 🔴 **E QUESTA RIGA DICEVA UNA COSA CHE È DIVENTATA FALSA NELLA STESSA GIORNATA.** Diceva: ~~«la correzione di oggi (`20260905000001`) **non è applicata da nessuna parte**: è scritta e non ha ancora girato su nessun database»~~. Era vera quando è stata scritta e ha smesso di esserlo poche ore dopo, quando quella migrazione è stata applicata al progetto di prova. **Misurato dalla coda di GitHub il 05/09**, non riferito: le prove contro il progetto di prova sono passate — **466 su 67 file**, sette in più delle 459 precedenti, che sono esattamente le sette prove RLS nuove — e quelle prove interrogano una funzione che quella migrazione crea. Se non fosse applicata, sarebbero rosse.
    - 🔴 **NON SI SCRIVE PIÙ, IN QUESTO DOCUMENTO, SE UNA MIGRAZIONE È APPLICATA O NO.** È la regola decisa il 31/08 e questa riga è il suo terzo esemplare: in un documento vivo una frase sullo stato marcisce, e marcisce senza avvisare. Lo stato lo dice il gestionale — `npm run migra` guarda il database vero — e per il progetto di prova lo dice la coda. Quello che resta scritto qui è la **regola**, che non invecchia; le fotografie stanno nei riepiloghi di `docs/consegne/`, dove la data è nel nome del file e una frase è una fotografia per costruzione.
